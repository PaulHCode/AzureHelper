...
#>
    ...
    Else {
        ...
        While ($Null -eq $sub -or $sub -is [array]) {
            $tenantLookup = (Get-AzTenant) | ForEach-Object { @{$_.id = $_.Name } }
            $sub = (Get-AzSubscription | Select-Object Name, Id, State, @{n = 'TenantName'; e = { $tenantLookup."$($_.TenantId)" } } , TenantId | Out-GridView -PassThru -Title "Select the subscription to use")
        }
    }
